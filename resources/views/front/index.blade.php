    ...
    <main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
        ...
        <section id="Brands" class="flex flex-col gap-[10px] mt-[30px] px-5">
            <h2 class="font-semibold text-lg leading-[27px]">By Brand</h2>
            <div class="grid grid-cols-3 gap-4">

                @forelse($brands as $brand)
                <a href="{{route('front.branding',$brand->slug)}}" class="card">
                    <div class="rounded-2xl ring-1 ring-[#EDEEF0] p-4 flex flex-col items-center gap-3 text-center transition-all duration-300 hover:ring-2 hover:ring-[#FCCF2F]">
                        <div class="w-[50px] h-[50px] flex shrink-0">
                            <img src="{{ Storage::url($brand->logo) }}" alt="icon" />
                        </div>
        ...
        <section id="New" class="flex flex-col gap-[10px] mt-[30px]">
            ...
            <div class="swiper w-full h-fit">
                <div class="swiper-wrapper">
                    ...
                    @forelse($latestProducts as $lProduct)
                    <a href="{{route('front.details', $lProduct->slug)}}" class="swiper-slide max-w-[150px] first-of-type:ml-5 last-of-type:mr-5">
                        <div class="flex flex-col gap-3 bg-white">
                            <div class="h-[130px] flex shrink-0 items-center rounded-2xl overflow-hidden bg-[#F6F6F6]">
                                <div class="h-[70px] w-full flex shrink-0 justify-center">
                                    <img src="{{ Storage::url($lProduct->thumbnail) }}" class="w-full h-full object-contain" alt="thumbnail" />
                                </div>
                            </div>
                            <div class="flex flex-col gap-1">
                                <p class="font-semibold break-words">{{ $lProduct->name }}</p>
                    ...
                    @empty
                    <p>Belum ada data brand terbaru</p>
                    @endforelse
        ...
        <section id="Recommendation" class="flex flex-col gap-[10px] mt-[30px] px-5">
            ...
            <div class="flex flex-col gap-5">
                ...
                @forelse($randomProducts as $rProduct)
                <a href="{{route('front.details', $rProduct->slug)}}" class="card">
                    <div class="flex items-center gap-3">
                        <div class="w-20 h-20 flex shrink-0 rounded-2xl overflow-hidden bg-[#F6F6F6] items-center">
                            <div class="w-full h-[50px] flex shrink-0 justify-center">
                                <img src="{{ Storage::url($rProduct->thumbnail) }}" class="h-full w-full object-contain" alt="thumbnail" />
                            </div>
                        </div>
                        <div class="w-full flex flex-col gap-1">
                            <p class="font-semibold">{{ $rProduct->name }}</p>
                            <div class="flex items-center justify-between">
                                <p class="text-sm leading-[21px] text-[#6E6E70]">RM {{ number_format($rProduct->price, 0, ',', '.') }}/Day</p>
